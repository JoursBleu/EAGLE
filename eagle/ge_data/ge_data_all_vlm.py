...
parser.add_argument('--data_file', type=str, default="/lpai/volumes/cloudmodel-muses/llava_data/LLaVA-Pretrain/blip_laion_cc_sbu_558k.json")
...
bigname="/lpai/volumes/lpai-demo-muses/lt/models/ad-vlm-models-24-03-14-1"
...
def build_dataset_rank(
    ...
    ds = load_dataset('json', data_files="/lpai/volumes/cloudmodel-muses/llava_data/LLaVA-Pretrain/blip_laion_cc_sbu_558k.json")
    ds = ds['train']
...
with open("/lpai/volumes/lpai-demo-muses/lt/llm_factory/blip/inference_results_0_30000.jsonl", 'r') as f:
    lines = f.readlines()

questions_ids = []
for ele in lines:
    questions_ids.append(json.loads(ele))

with open(args.data_file, 'r') as f:
    questions = json.load(f)
    length = len(questions)
    length = length if length < 30000 else 30000
    start = int((length) * args.index / 3.)
    end = int((length) * (args.index + 1) / 3.)
    # start = 0
    # end = 16
    questions = questions[start: end]
    questions_ids = questions_ids[start: end]

print("start", start, "end", end)
# breakpoint()

tokenizer, bigmodel, image_processor, context_len = load_pretrained_model(
            bigname, model_base="llava_mixtral", device_map="auto", load_in_8bit=True, load_in_4bit=False)
ds = create_data_loader(
    questions,
    "/lpai/volumes/cloudmodel-muses/llava_data/LLaVA-Pretrain/images",
    tokenizer,
    image_processor,
    bigmodel.config,
...
def ge(id, data):
    ...
    with torch.inference_mode():
        output_ids = questions_ids[id]['output_ids'][0]
        ...
        total_input_ids = torch.concat((input_ids, output_ids.to(input_ids.device)), dim=1)
        (
            inputs,
            position_ids,
            attention_mask,
    ...
    image_len = inputs_embeds.shape[1] - output_ids.shape[1]
    loss_mask=torch.zeros(inputs_embeds.shape[0],inputs_embeds.shape[1])
    for idx in range(image_len, inputs_embeds.shape[1]):
        loss_mask[0][idx] = 1
        
    # outs_big = bigmodel(total_input_ids.cuda(), output_hidden_states=True)
    ...
    td={"input_ids":total_input_ids.cpu()[0],"inputs_embeds":inputs_embeds.cpu()[0],"hidden_state":hidden_state_big.cpu()[0],"loss_mask":loss_mask.cpu()[0]}
    return td
